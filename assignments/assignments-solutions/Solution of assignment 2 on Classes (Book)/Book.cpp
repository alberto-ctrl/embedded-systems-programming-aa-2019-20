...
#include "Book.h"
	...
	bool Book::isbn_valid() const
	{
		...
		tokens.push_back(isbn_.substr(pos));	//serve a riempire tokens nel caso non entro nwl whiile
	...
	}	//se ritorna  1  il costruttore (sopra) non darà errore
...
std::string Book::genre_to_string(Book::Genre genre) const
{
	switch (genre)
	{
	case Genre::fiction:
	...
	case Genre::unknown:
		return "unknown";
	default:
		cerr << "Book::genre_to_string(): input is not a Genre\n";
		exit(EXIT_FAILURE);
	}
}
...
std::ostream& operator<<(std::ostream& os, const Book& b)
{
	os << "Title: " << b.title() << endl
		...
		<< "Checked out: " << boolalpha << b.checked_out() << endl;
	return os;
}
